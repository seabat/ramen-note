...
class ReportsRepository(
    ...
    override suspend fun load(): List<Report> = reportDao.getAllReportsDesc().map { it.toDomain() }

    override suspend fun insert(report: Report) {
        reportDao.insert(report.toEntity())
    }

    override suspend fun loadById(id: Int): Report? = reportDao.getReportById(id)?.toDomain()
