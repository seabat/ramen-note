...
3. **ソフトウェアキーボード表示中は画面下部のボタンが隠れて押せない**。`inputText` の直後にそのまま `tapOn` で下部のボタンを押そうとすると失敗する（ラベルタップやその他の方法で「キーボードを閉じたつもり」でも再度キーボードが出てくることがある）。
   - `hideKeyboard` コマンドはこのアプリでは `Couldn't hide the keyboard` で失敗するため使わない
   - キーボードの「done」（完了系）キーは**キーボードの言語レイアウトによって挙動が違う**。英語（QWERTY）キーボードの `tapOn: "done"` はキーボードを確実に閉じるが、**日本語かな入力キーボードの「完了」キーは IME の変換確定だけを行い、キーボード自体は閉じない**（実際に踏んだ事故：`完了` タップ後にキーボードが開いたままの状態で、次に「登録」ボタンのつもりでタップした座標がキーボード上の文字キーに当たり、フォームの文字列に意図しない1文字が混入した）
   - **最も確実な方法は、画面内の他の非インタラクティブなラベル（例：セクション見出しの「評価」など）を `tapOn` すること**。これはキーボード言語に関わらずフォーカスを外して確実にキーボードを閉じる。閉じたことは `inspect_screen` でキーボードのキー一覧（`あ`/`か`/`さ`... や `q`/`w`/`e`...）が消えたことで確認できる
   ```yaml
   - inputText: "..."
   - tapOn: "評価"               # 非インタラクティブなラベルをタップしてキーボードを閉じる（キー配列に依存しない）
   - waitForAnimationToEnd      # キーボードのdismissアニメーション完了を待つ
   # ここで inspect_screen し、キーボードのキー一覧が消えていることを確認してから次のタップに進む
   - tapOn: "送信ボタンのテキスト"
   ```
...
1. **登録**: 店舗詳細の 🍜追加（2つ並ぶ `追加` ボタンの2番目）→ 日付選択（デフォルト当日）、評価、メニュー情報、感想 → 「レポートする」
   ...
   - 「レポートする」ボタンはキーボードに隠れやすい。クセ3の手順（非インタラクティブなラベルをタップ → `waitForAnimationToEnd`）でキーボードを閉じてから押すこと
