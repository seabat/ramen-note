...
Gemini 2.5 系以降のモデルは、応答を生成する前に内部で「思考（推論）」を行い、その分の
トークンを消費する。この**思考トークンはユーザーに見えないが、出力トークンと同じ単価で課金**
されるため、単純なタスクでは大きなムダになる。

- **設定値の意味**（`ThinkingConfig#thinkingBudget`）:
  - `0` … 思考を**無効化**（モデルが対応している場合）
  - `-1` … **動的思考**（モデルがタスクに応じて予算を自動決定＝既定に近い挙動）
  - 正の整数 … 思考トークンの**上限**（範囲はモデル依存）
- 本アプリの店情報生成は「抽出＋分類」の単純タスクで内部推論を要さないため **`0`（無効化）** を採用。
  想定コストの約 75% を占めていた思考トークンをゼロにでき、**単一で最大の削減**になる。
- トレードオフ: 複雑な多段推論が必要なタスクでは思考 ON（`-1` 等）が品質に効くが、
  本アプリでは実機検証でも分類・description 品質の低下は確認されなかった。

#### `maxOutputTokens`（出力トークン上限）

モデルが 1 回の応答で生成できる最大トークン数。**既定は 0（無制限）**。

- **設定理由**: `description` が長文化すると高単価の出力トークンが増える。上限を設けることで
  1 回あたりの**最悪コストを固定**できる。
- **値の選定（512）**: description 2〜3 文の構造化 JSON に十分な余裕を持たせた値。
  ⚠️ **絞りすぎ厳禁**: 上限が小さいと JSON が途中で切れて parse に失敗し、`ShopAiInfo()`（空）が
  返る → ユーザーが再実行 → **かえって呼び出しが増える**逆効果になる。余裕を持たせること。
- **思考との関係**: 思考 ON の場合は思考トークンも `maxOutputTokens` を消費しうるため、
  上限が小さいと「思考だけで打ち切られ本文が空」になる罠がある。**思考 OFF ＋ 出力上限**の
  組み合わせが安全。
